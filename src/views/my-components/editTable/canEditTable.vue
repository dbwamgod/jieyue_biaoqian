...
<template>
    <div style="width: 1000px">
        <Table :ref="refs" height="200px" :columns="columnsList" :data="thisTableData" border disabled-hover></Table>
    </div>
</template>
...
const deleteButton = (vm, h, currentRow, index) => {
    if(vm.$store.state.app.editInlineAndCellData.length===1){
        return h('Button', {
            style: {
                margin: '0 5px',
            },
        ...
        on: {
            'on-ok': () => {
               /* if(!vm.$store.state.app.editInlineAndCellData.length===1){
                ...
                vm.$store.commit("delEditTable",index)
...
export default {
    ...
    watch: {
        ...
        "edittingStore"(to,form){
             // this.$store.commit("dataEditTable",to)
        }
